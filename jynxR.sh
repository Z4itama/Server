...
# $Id: start-nezumi.sh 29 2006-01-17 22:06:14Z MagicalTux $
#
...
# CONFIG
NEZUMI_PATH=.
NEZUMI_BIN="login-server_sql char-server_sql map-server_sql"
...
nezumi_resident() {
	...
	while true; do
		...
		for foo in $DAEMON; do
			...
			D_START=`echo "$foo" | cut -d'|' -f2`
			D_PID=`echo "$foo" | cut -d'|' -f3`
			LDIR=./log/$D_NAME-$(date +%Y:%m:%d:%H:%M:%S).log
			#LDIR=./log/testtesttest
			UPTIME=$[ $NOW - $D_START ]
			if [ x$D_PID = x ]; then
				# start daemon !
