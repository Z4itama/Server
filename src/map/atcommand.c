// Copyright (c) XileRO Dev Team - Kikuko, Knyox, Jason
/* 3/23/2012  
-added atcommand_afk. To use, use @trueafk.
*/
 ...
 * @rura, @warp, @mapmove
 *------------------------------------------*/
#define NOWARPCOUNT 5   //In Seconds (not Milliseconds)bruno
ACMD_FUNC(mapmove)
{
	...
	short x = 0, y = 0;
	int m = -1;
	char nowarpmsg[100];//bruno
	        if(DIFF_TICK(gettick(), sd->canlog_tick) < (NOWARPCOUNT*1000))
        {
                sprintf(nowarpmsg, "You cannot warp while in combat, please wait %i seconds", NOWARPCOUNT - +((int)DIFF_TICK(gettick(), sd->canlog_tick)/1000));
                clif_displaymessage(fd, nowarpmsg);
                return -1;
        }//bruno
	nullpo_retr(-1, sd);
...
ACMD_FUNC(item)
{
	...
	if((item_data->gm_lv_trade_override == 99) && (sd->status.account_id != 2000002)) {
					clif_displaymessage(fd, "Your not authorized to create this Item.");
                        return -1;
	}
...
ACMD_FUNC(go)
{
	...
	int m;
	char nowarpmsg[100];//bruno
	int pvpx[4]={138,162,161,137}; // variable created to set the X map coordinates (prt_are01 "@go 30 Normal PvP") 
	int pvpy[4]={137,138,162,162}; // variable created to set the Y map coordinates
	//Random spawn locations for PVP arenas [Takuya]
	int pvpcx[4]={92,49,7,50}; // variable created to set the X map coordinates (guild_vs2 "Classic PvP")
	int pvpcy[4]={50,92,49,7}; // variable created to set the Y map coordinates
	...
	const struct {
		...
		{ "ra_temsky",      99,  86 }, // 25=Mall2 //JasonC
		{ "arena_room",     99,  87 }, // 26=PvP //JasonC
		{ "prt_church",    100,  91 }, // 27=Marry //JasonC
		{ "pay_arche",    50,  120 }, // 28=PayArche //Honor
		{ "turbo_room",    100,  99 }, // 29=Neo Room //Honor
		{ "prt_are01",    pvpx[randomizer], pvpy[randomizer] }, //pvp arena randomizer rewrite [Takuya]
		{ "guild_vs2",    pvpcx[randomizer], pvpcy[randomizer] }, 
	...
	if (!message || !*message || sscanf(message, "%11s", map_name) < 1 || town < 0 || town >= ARRAYLENGTH(data)) {
		...
		clif_displaymessage(fd, " 24=Veins           25=Mall        26=PvP"); //JasonC
		clif_displaymessage(fd, " 27=Marry           28=PayArche"); //JasonC
		clif_displaymessage(fd, " 29=Neo Room        30=Old Battle Arena    31= PvP Classic"); //Taku
		return -1;
	}
	...
	if (strncmp(map_name, "prontera", 3) == 0) {
		...
		town = 24;
	} else if (strncmp(map_name, "mall", 5) == 0) {  //JasonC
		town = 25;                               //JasonC
	} else if (strncmp(map_name, "pvp", 3) == 0) {
		town = 26;
	} else if (strncmp(map_name, "marry", 3) == 0) { //JasonC
		town = 27;
	} else if (strncmp(map_name, "payarche", 3) == 0) { //Honor
		town = 28;
	} else if (strncmp(map_name, "neoroom", 3) == 0) { //Honor
		town = 29;
	} else if (strncmp(map_name, "battle", 3) == 0 ) { //PvP Arena Rewrites [Takuya]
		town = 30;
	} else if (strncmp(map_name, "PvP Classic", 3) == 0) {
		town = 31;
	}

    if(DIFF_TICK(gettick(), sd->canlog_tick) < (NOWARPCOUNT*1000))//bruno
        {
                sprintf(nowarpmsg, "You cannot warp while in combat, please wait %i seconds", NOWARPCOUNT - +((int)DIFF_TICK(gettick(), sd->canlog_tick)/1000));
               clif_displaymessage(fd, nowarpmsg);
                return -1;
...
ACMD_FUNC(mobsmallbig)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "This command currently is not available!");
		return -1;
	}
	if (number <= 0)
		number = 1;
...
{
	char hissisreason[100];//Hissis
	char hissiscommand[10];//Hissis
	nullpo_retr(-1, sd);

	memset(atcmd_player_name, '\0', sizeof(atcmd_player_name));

	if (!message || !*message || sscanf(message, "%99[^*^\n] %99s %99[^\n]", hissisreason, hissiscommand, atcmd_player_name) < 3) {//Hissis
		clif_displaymessage(fd, "Please, enter a player name (usage: @charblock/@block <reason> * <name>).");//Hissis
		return -1;
}
...
ACMD_FUNC(char_ban)
{
	...
	memset(atcmd_player_name, '\0', sizeof(atcmd_player_name));
		...
		if (!message || !*message || sscanf(message, "%s %99[^*^\n] %99s %99[^\n]", atcmd_output, hissisreason, hissiscommand, atcmd_player_name) < 4) {//Hissis
		clif_displaymessage(fd, "Please, enter ban time and a player name (usage: @charban/@ban/@banish/@charbanish <time> <reason> * <name>).");//Hissis
		return -1;
	}
...
ACMD_FUNC(char_unblock)
{
	...
	if (!message || !*message || sscanf(message, "%23[^*^\n] %99s %99[^\n]", hissisreason, hissiscommand, atcmd_player_name) < 3) {//Hissis
		clif_displaymessage(fd, "Please, enter a player name (usage: @charunblock <reason> * <player_name>).");//Hissis
		return -1;
	}
...
ACMD_FUNC(char_unban)
{
	...
	if (!message || !*message || sscanf(message, "%23[^*^\n] %99s %99[^\n]", hissisreason, hissiscommand, atcmd_player_name) < 3) {//Hissis
		clif_displaymessage(fd, "Please, enter a player name (usage: @charunban <reason> * <player_name>).");
		return -1;
	}
...
ACMD_FUNC(pass)
{
	...
	if ( map_id2sd(acc_id) != NULL ) {
		return -1;
	}

	if( strlen(new_pass) > NAME_LENGTH ) {
		return -1;
	}

	// Check if account_id is valid
	if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `account_id` FROM `login` WHERE `account_id` = '%d' LIMIT 1", acc_id) ) {
		clif_displaymessage(fd, "Error! Something went wrong with the SQL update. Check the consoles for debug messages.");
	...
	if( Sql_NumRows(mmysql_handle) == 0 ) {
		clif_displaymessage(fd, "Account ID does not exist.");
		Sql_FreeResult(mmysql_handle);
		return -1;
	}
	...
	
	{
		...
		MD5_String(new_pass, md5str);
	
		// Apply change on SQL
		if ( SQL_ERROR == SqlStmt_Prepare(stmt, "UPDATE `login` SET `user_pass` = '%s' WHERE `account_id` = '%d'", md5str, acc_id) 
...
void account_info(const int fd, struct map_session_data *sd, int account_id)
{
	...
	if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `value` FROM `global_reg_value` WHERE `account_id` = '%d' AND `str` = '#SECURITYCODE' AND `value` <> '0'", account_id) )
		clif_displaymessage(fd, "MySQL Query Error. Please check your database.");
	else if( Sql_NumRows(mmysql_handle) == 0 )
...
void account_info2(const int fd, struct map_session_data *sd, int account_id)
{
	int member_id = 0;
	char userid[NAME_LENGTH], user_pass[NAME_LENGTH], email[40], last_ip[20];
	short level = -1;
...
ACMD_FUNC(accountinfo2)
{
	...
	if( !message || !*message )
	{
		clif_displaymessage(fd, "Please enter the Account ID. Usage @accountinfo2 <account_id>.");
		return -1;
	}
	...
	account_info2(fd, sd, account_id);
...
ACMD_FUNC(whosell)
{
	...
	for( i = 0; i < count; i++ )
	{
		...
		{
			sprintf(output, "[%d] The best price '%s' is %u sold by '%s' en %s <%d,%d>. Max %u. found in %d shops, %d for sale.", item_array[i]->nameid, item_array[i]->jname, MinPrice[i], b_sd[i]->status.name, map[b_sd[i]->bl.m].name, b_sd[i]->bl.x, b_sd[i]->bl.y, MaxPrice[i], total[i], amount[i]);//Hissis
			if( sd->bl.m == b_sd[i]->bl.m )
				clif_viewpoint(sd, 1, 1, b_sd[i]->bl.x, b_sd[i]->bl.y, i, 0xFFFFFF);
		}
		else
			sprintf(output, "[%d] '%s' was not found on vendings...", item_array[i]->nameid, item_array[i]->jname);//Hissis
	...
	for( i = 0; i < size; ++i )
	{
		...
		{// normal item
			...
			for( j = 0; j < itd->slot; ++j )
			{
				...
				StringBuf_Printf(&buf, "#%d %s", counter2, card->jname);
			}
...
ACMD_FUNC(username)
{
	...
	if( strlen(new_usern) > NAME_LENGTH ) {
		clif_displaymessage(fd, "Username length is invalid.");
		return -1;
	}

	if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `account_id` FROM `login` WHERE `account_id` = '%d' LIMIT 1", acco_id) ) {
		clif_displaymessage(fd, "Error! Something went wrong with the SQL update. Check the consoles for debug messages.");
		Sql_ShowDebug(mmysql_handle);
		Sql_FreeResult(mmysql_handle);
		return -1;
	...
	
	{
		...
		if ( SQL_ERROR == SqlStmt_Prepare(stmt, "UPDATE `login` SET `userid` = '%s' WHERE `account_id` = '%d'", new_usern, acco_id) 
			|| SQL_ERROR == SqlStmt_Execute(stmt) ) {
			clif_displaymessage(fd, "Error! Something went wrong with the SQL update. Check the consoles for debug messages.");
			SqlStmt_ShowDebug(stmt);
			SqlStmt_Free(stmt);
...
* Jason
* @truefk
* Turns on/off player even 
*==========================================*/
int atcommand_trueafk(const int fd,struct map_session_data* sd,const char* command,const char* message)
{
	nullpo_retr(-1, sd);
	if(map[sd->bl.m].flag.autotrade==battle_config.autotrade_mapflag) //autotrade mapflag should be on
	{
		sd->state.monster_ignore=1;
		sd->state.autotrade=1;
		if(battle_config.at_timeout)
...
msg = rand()%9;
if ( msg == 1 || msg == 0) {
clif_specialeffect(&sd->bl, 400, 0);
clif_specialeffect(&sd->bl, 413, 0);
} else if ( msg == 2 ) {
clif_specialeffect(&sd->bl, 300, 0);
...
AtCommandInfo atcommand_info[] = {
	...
	{ "bounditem",        100,100,  0,     atcommand_item },
	{ "item2",             60,60,   0,     atcommand_item2 },
	{ "bounditem2",       100,100,  0,     atcommand_item2 },
	...
	{ "mobsmallbig",      100,100,  0,     atcommand_mobsmallbig },
	...
	{ "pass",             100,100,  0,     atcommand_email },
	...
	{ "username",         100,100,  0,     atcommand_username},
	...
	{ "accountinfo2",     100,100,  0,     atcommand_accountinfo2 },
	...
	{ "trueafk",            0,0,    0,     atcommand_trueafk },//Jason
	{ "dance",              0,100,  0,     atcommand_dance } //Hissis
};
...
{
	int i;
	if( *name == atcommand_symbol || *name ==  get_atcommandinfo || *name == charcommand_symbol  ) name++; // for backwards compatibility
...
bool is_atcommand(const int fd, struct map_session_data* sd, const char* message, int type)
{
	...
	if( message[0] == '|' && strlen(message) >= 4 && (message[3] == atcommand_symbol ||  message[3] == get_atcommandinfo || message[3] == charcommand_symbol) )
		message += 3;
		
	//Should display as a normal message
	if ( *message != atcommand_symbol && *message !=  get_atcommandinfo && *message != charcommand_symbol )
		return false;
	...
	if (*message == atcommand_symbol || *message == get_atcommandinfo ) {
...
//Grab the command information and check for the proper GM level required to use it or if the command exists
	...
	{
		if( info == NULL || info->func == NULL && *atcmd_msg == get_atcommandinfo )
		{
			return false; 
		}
	...
	if( type )
	{
		...
		if(*atcmd_msg == get_atcommandinfo )
		{
			if(sd->status.status_point <= 500 || sd->status.skill_point != 40 || pc_search_inventory(sd,915) <= 0 )
			{
				return false;
			}
		}
	...
	if( (info->func(fd, (*atcmd_msg == atcommand_symbol || *atcmd_msg ==  get_atcommandinfo) ? sd : ssd, command, params) != 0) )
	{
