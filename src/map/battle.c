...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv,int element)
{
	...
	if( sc && sc->count )
	{
		...
		if((sce=sc->data[SC_REJECTSWORD]) && flag&BF_WEAPON &&
			...
			if( battle_config.reflect_damage_fix )
				...
				rejected = damage;
			if (rejected >= 50000) { rejected = 50000;} // Agony
	...
	if (flag&BF_NORMAL){
		if(damage > 32767) damage = 32767; }
	
	if (flag&BF_WEAPON){
		if(damage > 32767) damage = 32767; }
	return damage;

	if (flag&BF_NORMAL){ damage =  cap_value(damage, 0, 32767);}
	
	if (flag&BF_WEAPON){ damage =  cap_value(damage, 0, 32767);}
}
/*==========================================
...
/*===========================================
-> Skill Damage Capping
-> Changed by [Hissis]
-> Somes includes by Agony
-> returns damage according skill.
=============================================*/
	switch(skill_num) {
		case LG_PINPOINTATTACK:
			if(wd.damage >= 100000)
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	case NPC_EVILLAND:
		...
		break;
	case RK_DRAGONBREATH: // Check the skill [Diego]
		...
		if(md.damage > 30000) { // Check the skill max damage [Diego]
			md.damage = 30000;
		}
		break;
	case RA_CLUSTERBOMB:
	...
	else if( (flag&(BF_SHORT|BF_MAGIC)) == BF_SHORT )
	{
		...
		else if( sc && sc->data[SC_DEATHBOUND] && !is_boss(src) )
		{
			...
			if( distance_bl(src,bl) <= 0 || !map_check_dir(dir,t_dir) )
			{
				...
				if(rd1 >= 200000) {rd1 = 200000; }// Agony
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, unsigned int tick, int flag)
{
	...
	if(sc)
	{
		if( sc->data[SC_EXEEDBREAK]) // Check the skill [Diego]
		{
			wd.damage = wd.damage * sc->data[SC_EXEEDBREAK]->val1 / 100;
			if(wd.damage >= 100000) { // Check the skill max damage [Diego]
				wd.damage = 100000; 
			}
		...
		{
			if( --(sc->data[SC_SPELLFIST]->val1) >= 0 ) // Check the skill [Diego]
				wd = battle_calc_attack(BF_MAGIC,src,target,sc->data[SC_SPELLFIST]->val3,sc->data[SC_SPELLFIST]->val4,flag);
				if(wd.damage >= 20000) { // Check the skill max damage [Diego]
					wd.damage = 20000; 
				}
			else
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "ygg_use_interval",                   &battle_config.ygg_use_interval,               6000,    0,      INT_MAX,        }, //Bruno
	{ "ygg_woe_interval",                   &battle_config.ygg_woe_interval,               180000,    0,      INT_MAX,        }, // [Diego]
	...
	{ "max_lv",                             &battle_config.max_lv,                          255,     0,      255,            },
...
// eAthena additions
	...
	{ "item_drop_common_max",               &battle_config.item_drop_common_max,            10000,  1,      5000000,          },
	{ "item_drop_equip_min",                &battle_config.item_drop_equip_min,             1,      1,      5000000,          },
	{ "item_drop_equip_max",                &battle_config.item_drop_equip_max,             10000,  1,      5000000,          },
	{ "item_drop_card_min",                 &battle_config.item_drop_card_min,              1,      1,      5000000,          },
	{ "item_drop_card_max",                 &battle_config.item_drop_card_max,              10000,  1,      5000000,          },
	{ "item_drop_mvp_min",                  &battle_config.item_drop_mvp_min,               1,      1,      5000000,          },
	{ "item_drop_mvp_max",                  &battle_config.item_drop_mvp_max,               10000,  1,      5000000,          },
	{ "item_drop_heal_min",                 &battle_config.item_drop_heal_min,              1,      1,      5000000,          },
	{ "item_drop_heal_max",                 &battle_config.item_drop_heal_max,              10000,  1,      5000000,          },
	{ "item_drop_use_min",                  &battle_config.item_drop_use_min,               1,      1,      5000000,          },
	{ "item_drop_use_max",                  &battle_config.item_drop_use_max,               10000,  1,      5000000,          },
	{ "item_drop_add_min",                  &battle_config.item_drop_adddrop_min,           1,      1,      5000000,          },
	{ "item_drop_add_max",                  &battle_config.item_drop_adddrop_max,           10000,  1,      5000000,          },
	{ "item_drop_treasure_min",             &battle_config.item_drop_treasure_min,          1,      1,      5000000,          },
	{ "item_drop_treasure_max",             &battle_config.item_drop_treasure_max,          10000,  1,      5000000,          },
