...
int npc_cashshop_buylist(struct map_session_data *sd, int points, int count, unsigned short* item_list)
{
	...
	{ // Cash Shop
		int value1 = pc_readregistry(sd,nd->u.shop.cash_var,nd->u.shop.cash_vartype),
			value2 = ( nd->u.shop.cash_vartype != -1 ) ? pc_readregistry(sd,nd->u.shop.point_var,nd->u.shop.point_vartype) : 0;

		if( value2 < points || value1 < (vt - points) )
			return 6;

		pc_setregistry(sd,nd->u.shop.cash_var,value1 - (vt - points),nd->u.shop.cash_vartype);
		if( nd->u.shop.cash_vartype != -1 ) pc_setregistry(sd,nd->u.shop.point_var,value2 - points,nd->u.shop.point_vartype);
	}
	// -------------------------------------------------------------------
