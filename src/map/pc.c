...
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int gmlevel, struct mmo_charstatus *st)
{
	...
	sd->canuseygg_tick = tick; //Bruno
	sd->canuseyggwoe_tick = tick; // [Diego]
...
{
	/* clif_displaymessage(sd->fd, "Your item will disappear.");
	pc_delitem(sd, n, amount, 0, 1);
	return 1; */
...
int pc_useitem(struct map_session_data *sd,int n)
{
	...
	if( sd->sc.count && (
		...
		return 0;
   // Delay de 3 segundos nas Yggs.[Bruno]
    if((sd->status.inventory[n].nameid == 608 || sd->status.inventory[n].nameid == 607) && +DIFF_TICK(sd->canuseygg_tick, tick) > 0)
       return 0;
    else
        sd->canuseygg_tick= tick + battle_config.ygg_use_interval;//bruno
	// Store information for later use before it is lost (via pc_delitem) [Paradox924X]
	nameid = sd->inventory_data[n]->nameid;
	
	// Ygg delay WOE [Diego]
		if((sd->status.inventory[n].nameid == 608 || sd->status.inventory[n].nameid == 607) && +DIFF_TICK(sd->canuseyggwoe_tick, tick) > 0 && agit_flag && map[sd->bl.m].flag.gvg_castle)
       return 0;
    else
        sd->canuseyggwoe_tick= tick + battle_config.ygg_woe_interval;
 ...
 * [Hissis]
 *------------------------------------------*/
int pc_countitem(struct map_session_data *sd, int item_id)
{
	struct item_data* id;
	int i, nameid, count = 0;
	...
	if( id == NULL ) {
		ShowWarning("pc_countitem: Invalid item '%s'.\n", item_id);
		return 0;
	}
 ...
 * [Hissis]
 *------------------------------------------*/
bool pc_validdelay(struct map_session_data *sd, const char* str, int delay)
{
	int curr_time, tick;
	
	nullpo_retr(false, sd);
	
	curr_time = pc_readglobalreg(sd, str);
	tick = (int)time(NULL);
	
	return (curr_time != 0 && curr_time >= tick);
}

/*==========================================
 * [Hissis]
 *------------------------------------------*/
int pc_setdelay(struct map_session_data *sd, const char* str, int delay)
{
	int tick;
	...
	tick = (int)time(NULL);
	pc_setglobalreg(sd, str, tick + delay);
...
int pc_equipitem(struct map_session_data *sd,int n,int req_pos)
{
	...
	if(pos & EQP_HAND_R) {
		...
		pc_calcweapontype(sd);
		clif_changelook(&sd->bl,LOOK_WEAPON,sd->status.weapon);
		status_change_end(&sd->bl, SC_DANCING, INVALID_TIMER); //Hissis
	}
	if(pos & EQP_HAND_L) {
		if(id) {
