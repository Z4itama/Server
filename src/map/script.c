...
bool ctf_flag[CTF_ARENAS][CTF_TEAMS][CTF_FLAGS];
...
static int pet_return_egg(struct map_session_data *sd, struct pet_data *pd)
{
	...
	//if((flag = pc_additem(sd,&tmp_item,1,LOG_TYPE_SCRIPT))) {
...
BUILDIN_FUNC(warpparty)
{
	...
	else
	{
		...
		for (i = 0; i < MAX_PARTY; i++)
		{
			if ((pl_sd = p->data[i].sd))
			{
				//if(map[pl_sd->bl.m].flag.noreturn || map[pl_sd->bl.m].flag.nowarp) //knx
					//continue; //knx
				if(pc_isdead(pl_sd)) status_revive(&pl_sd->bl, 1, 1); //knx
				pc_setpos(pl_sd,mapindex,x,y,3);
			}
		}
	}
...
BUILDIN_FUNC(getmapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_INDOORS:			script_pushint(st,map[m].flag.indoors); break;//bruno
...
BUILDIN_FUNC(setmapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_INDOORS:			map[m].flag.indoors=1; break;//bruno
...
//knx
BUILDIN_FUNC(getpartymembercount)
{
	struct party_data *p;
	TBL_PC* sd = NULL;
	int i,j=0,type=0;
	...
	if(p!=NULL){
		for(i=0;i<MAX_PARTY;i++){
			if(p->party.member[i].account_id){
				...
				if(sd && sd->status.char_id != p->party.member[i].char_id) sd = NULL;
...
//knx
BUILDIN_FUNC(countpartyin)
{
	struct party_data *p;
	TBL_PC* sd = NULL;
	const char *str;
	int i,j=0,type=0,mymap=0;

	str = script_getstr(st,2);
	mymap = mapindex_name2id(str);
	p=party_search(script_getnum(st,3));
...
BUILDIN_FUNC(party_leave)
{
	...
	if( i == MAX_PARTY ) return 0; // request from someone not in party? o.O
	
	//Change leadership to someone else if necessary
	if( safe && p->party.member[i].leader ){
		ARR_FIND( 0, MAX_PARTY, i, p->party.member[i].account_id != 0 && p->party.member[i].account_id != sd->status.account_id );
		if( i != MAX_PARTY ) // there's someone else in the party
			intif_party_leaderchange(sd->status.party_id, p->party.member[i].account_id, p->party.member[i].char_id);
	}
...
//knx
BUILDIN_FUNC(party_join)
{
	int leaderpartyid;
	struct party_member member;
	const char *type;
	TBL_PC* sd = NULL;
	TBL_PC* sd_leader = NULL;

	type = script_getstr(st,2);
	leaderpartyid = script_getnum(st,3);
	
	sd = map_id2sd(st->rid);
	if(sd==NULL) return 0;

	if(strcmp(type,"leader")==0){
		sd_leader = map_id2sd(leaderpartyid);
		if(sd_leader==NULL) return 0;
	
		party_fill_member(&member, sd);
		intif_party_addmember(sd_leader->status.party_id, &member);
		script_pushint(st, sd_leader->status.party_id);
	}else if(strcmp(type,"party")==0){
		if(!party_search(leaderpartyid)) return 0;
		party_fill_member(&member, sd);
		intif_party_addmember(leaderpartyid, &member);
		script_pushint(st, leaderpartyid);
	}
...
BUILDIN_FUNC(party_break)
{
	...
	if( i == MAX_PARTY ) return 0; // no leader? o.O
	
	//leader leaves => party's gone
	intif_party_leave(pid,p->party.member[i].account_id,p->party.member[i].char_id);

	return 0;
}

//knx
BUILDIN_FUNC(ctf_init)
{
	int i,j, ctf_arena;
	ctf_arena = script_getnum(st,2);

	if(ctf_arena >= 0 && ctf_arena < CTF_ARENAS)
		for(i=0;i<CTF_TEAMS;i++)
			for(j=0;j<CTF_FLAGS;j++)
				ctf_flag[ctf_arena][i][j] = 0;

	return 0;
}

//knx
BUILDIN_FUNC(ctf_lockflag)
{
	TBL_PC* sd = NULL;
	int a, t, f;
	...
	sd = map_id2sd(st->rid);
	a = script_getnum(st,2);
	t = script_getnum(st,3) - 1;
	f = script_getnum(st,4) - 1;

	if(a >= 0 && a < CTF_ARENAS && t >= 0 && t < CTF_TEAMS && f >= 0 && f < CTF_FLAGS){
		if(ctf_flag[a][t][f]==0){
			ctf_flag[a][t][f]=1;
			if(sd) sd->state.ctf_active=1;
			script_pushint(st, 1);
		}else{
			script_pushint(st, 0);
		}
...
BUILDIN_FUNC(ctf_unlockflag)
{
	...
	if(a >= 0 && a < CTF_ARENAS && t >= 0 && t < CTF_TEAMS && f >= 0 && f < CTF_FLAGS){
		ctf_flag[a][t][f]=0;
		if(sd) sd->state.ctf_active=0;
	}

	if(sd && a==0 && t==-1 && f==-1) sd->state.ctf_active=0;
...
BUILDIN_FUNC(sc_get)
{
	...
	if(sc->data[type]) script_pushint(st, 1);
...
//knx
BUILDIN_FUNC(arena_queue_add)
{
	struct map_session_data *sd,*sd2;
	struct party_data *p;
	char out[256],*reg;
	int len,type,i,x=0,y=0;
	sd = map_id2sd(st->rid);
	type = script_getnum(st,2);
	...
	if(type==1){
		if(sd->my_are_fifo1!=NULL || sd->my_are_fifo2!=NULL || sd->my_are_fifo3!=NULL){
			script_pushint(st, -1);
			return 0;
		}else if(fifo_new(sd,0)==0){
			//success
			sd->my_are_fifo1=are_queue_last[0];
			sprintf(out, "Successfully registered on 5manFFA. You have %d people ahead of you.", are_queue_length[0]-1); len=strlen(out);
			clif_disp_onlyself(sd,out,len);
			script_pushint(st, are_queue_length[0]);
			return 0;
		}
	}else if(type==2){
		x=pc_readreg(sd, search_str("@PRTYPRTY_x"));
		y=pc_readreg(sd, search_str("@PRTYPRTY_y"));
		reg=pc_readregstr(sd, search_str("@PRTYPRTY_city$"));
		p=party_search(sd->status.party_id);
		if(p!=NULL){
			for(i=0;i<MAX_PARTY;i++){
				if(p->party.member[i].account_id){
					sd2 = map_id2sd(p->party.member[i].account_id);
					if(sd2 && (sd2->my_are_fifo1!=NULL || sd2->my_are_fifo2!=NULL || sd2->my_are_fifo3!=NULL)){ script_pushint(st, -1); return 0; }
			...
			if(fifo_new(sd,1)==0){
				for(i=0;i<MAX_PARTY;i++){
					if(p->party.member[i].account_id){
						...
						sd2->my_are_fifo2=are_queue_last[1];
						pc_setreg(sd2, add_str("@PRTYPRTY_active"), 1);
						if(sd2!=sd && reg){
							pc_setreg(sd2,add_str("@PRTYPRTY_x"),x);
							pc_setreg(sd2,add_str("@PRTYPRTY_y"),y);
							pc_setregstr(sd2, add_str("@PRTYPRTY_city$"), reg);
						}
						sprintf(out, "Successfully registered on 3vs3. You have %d parties ahead of you.", are_queue_length[1]-1); len=strlen(out);
						clif_disp_onlyself(sd2,out,len);
					}
				}
				script_pushint(st, are_queue_length[1]);
				return 0;
			}
		}
	}else if(type==3){
		if(sd->my_are_fifo1!=NULL || sd->my_are_fifo2!=NULL || sd->my_are_fifo3!=NULL){
			script_pushint(st, -1);
			return 0;
		}else if(fifo_new(sd,2)==0){
			//success
			sd->my_are_fifo3=are_queue_last[2];
			sprintf(out, "Successfully registered on Capture the flag. You have %d people ahead of you.", are_queue_length[2]-1); len=strlen(out);
			clif_disp_onlyself(sd,out,len);
			script_pushint(st, are_queue_length[2]);
			return 0;
		}
	}
...
BUILDIN_FUNC(arena_queue_del)
{
	...
	if(type==1){
		if(sd->my_are_fifo1!=NULL && fifo_del(sd->my_are_fifo1,0)==0)
			sd->my_are_fifo1=NULL; //success
	}else if(type==2){
		sprintf(out, "Someone from the Party logged out. You were removed from 3vs3's queue."); len=strlen(out);
		if(sd->my_are_fifo2!=NULL && fifo_del(sd->my_are_fifo2,1)==0)
			sd->my_are_fifo2=NULL;
		p=party_search(sd->status.party_id);
		if(p!=NULL){
			for(i=0;i<MAX_PARTY;i++){
				if(p->party.member[i].account_id){
					sd2 = map_id2sd(p->party.member[i].account_id);
					if(sd2==NULL) continue;
					if(sd2->my_are_fifo2!=NULL) clif_disp_onlyself(sd2,out,len);
					sd2->my_are_fifo2=NULL;
					pc_setreg(sd2, add_str("@PRTYPRTY_active"), 0);
				}
			}
		}
	}else if(type==3){
		if(sd->my_are_fifo3!=NULL && fifo_del(sd->my_are_fifo3,2)==0)
			sd->my_are_fifo3=NULL; //success
	}
...
//knx
BUILDIN_FUNC(warp_arena_queue)
{
	struct map_session_data *sd,*sd2;
	struct party_data *p;
	struct arena_fifo* fifo=NULL;
	const char* str;
	char out[256],*reg;
	int i,j,x,y,maxp,ret,len,type,arena,t[2];

	sd = map_id2sd(st->rid);
	type = script_getnum(st,2);
	arena = script_getnum(st,3);
	str = script_getstr(st,4);
	x = script_getnum(st,5);
	y = script_getnum(st,6);
	maxp = script_getnum(st,7);

	if(type==1){
		//FIVE MAN FREE FOR ALL-------------------------------------------------------
		if(are_queue_length[0] < maxp){
			script_pushint(st, 0);
			return 0;
		}

		for(i=0 ; i<maxp ; i++){
			if(are_queue_first[0]==NULL){ break; }
			if(are_queue_first[0]->sd->pd!=NULL) pet_return_egg(are_queue_first[0]->sd, are_queue_first[0]->sd->pd);
			ret = pc_setpos(are_queue_first[0]->sd,mapindex_name2id(str),x,y,0);
			//if(ret) i--;
			are_queue_first[0]->sd->my_are_fifo1=NULL;
			fifo_del(are_queue_first[0],0);
		}

		j=0;
		fifo=are_queue_first[0];
		len=strlen("You have 10000 people ahead of you to enter 5mFFA.");
		while(fifo!=NULL){
			if(fifo->sd==NULL){ fifo=fifo->next; continue; }
			sprintf(out, "You have %d people ahead of you to enter 5mFFA.", j);
			clif_disp_onlyself(fifo->sd,out,len);
			fifo=fifo->next; j++;
		}

		script_pushint(st, i);
	}else if(type==2){
		//THREE VS THREE PARTY--------------------------------------------------------
		if(are_queue_length[1] < maxp){
			script_pushint(st, 0);
			return 0;
		}

		//todo: make it fill t[0] upto t[maxp]?
		t[0]=rand()%4;
		t[1]=(t[0]+((rand()%3)+1))%4;

		for(i=0 ; i<maxp ; i++){
			if(are_queue_first[1]==NULL){ break; }
			p=party_search(are_queue_first[1]->sd->status.party_id);
			if(p!=NULL){
				//Get Respawn point
				x=pc_readreg(are_queue_first[1]->sd, search_str("@PRTYPRTY_x"));
				y=pc_readreg(are_queue_first[1]->sd, search_str("@PRTYPRTY_y"));
				reg=pc_readregstr(are_queue_first[1]->sd, search_str("@PRTYPRTY_city$"));
				//Global map vars.
				sprintf(out,"$PP%d_Team%d_id",arena,i+1);
					mapreg_setreg(add_str(out), are_queue_first[1]->sd->status.party_id);
				sprintf(out,"$PP%d_Team%d_x",arena,i+1);
					mapreg_setreg(add_str(out), x);
				sprintf(out,"$PP%d_Team%d_y",arena,i+1);
					mapreg_setreg(add_str(out), y);
				sprintf(out,"$PP%d_Team%d_city$",arena,i+1);
					mapreg_setregstr(add_str(out), reg);
				//spawning coords (guild_vs1* maps)
				switch(t[i]){
					case 0: x=91; y=49; break;
					case 1: x=49; y=91; break;
					case 2: x= 8; y=49; break;
					case 3: x=49; y= 8; break;
				}
				//warp party
				for(j=0;j<MAX_PARTY;j++){
					if(p->party.member[j].account_id){
						sd2 = map_id2sd(p->party.member[j].account_id);
						if(sd2==NULL) continue;
						if(sd2->pd!=NULL) pet_return_egg(sd2, sd2->pd);
						ret=pc_setpos(sd2,mapindex_name2id(str),x,y,0);
						//if(ret) i--;
						sd2->my_are_fifo2=NULL;
					}
				}
			}else{
				i--; //shouldnt happen but, just in case, try to get another party in
			}
			fifo_del(are_queue_first[1],1);
		}

		j=0;
		fifo=are_queue_first[1];
		len=strlen("You have 10000 people ahead of you to enter 3vs3.");
		while(fifo!=NULL){
			if(fifo->sd==NULL){ fifo=fifo->next; continue; }
			sprintf(out, "You have %d people ahead of you to enter 3vs3.", j);
			clif_disp_onlyself(fifo->sd,out,len);
			fifo=fifo->next; j++;
		}
		...
		//we use "maxp = -partyid" to add a new member to that party
		if(maxp < 0){
			if(sd==NULL) return 0;
			//get partyid
			maxp = -maxp;
			//checks
			if(are_queue_first[2]==NULL) return 0;
			if(are_queue_first[2]->sd->pd!=NULL) pet_return_egg(are_queue_first[2]->sd, are_queue_first[2]->sd->pd);
			//auto party-management
			if(are_queue_first[2]->sd->status.party_id){ party_leave(are_queue_first[2]->sd); are_queue_first[2]->sd->status.party_id=0; }
			//party_fill_member(&member, are_queue_first[2]->sd);
			//intif_party_addmember(maxp, &member);
			//set @CTF_team
			i = pc_readreg(sd, search_str("@CTF_team"));
			if(i<1 || i>2){ if(y>100) i=1; else i=2; }
			pc_setreg(are_queue_first[2]->sd, add_str("@CTF_team"), i);
			//arena var
			pc_setreg(are_queue_first[2]->sd, add_str("@CTF_arena"), arena);
			//warp player
			pc_setpos(are_queue_first[2]->sd,mapindex_name2id(str),x,y,0);
			//queue
			are_queue_first[2]->sd->my_are_fifo3=NULL;
			fifo_del(are_queue_first[2],2);
			return 0;
		}

		//otherwise, maxp = number of players that need to be split into two parties and brought to the arena
		if(are_queue_length[2] < maxp){
			script_pushint(st, 0);
			return 0;
		}

		//search for unused party names (use "CTF"+tick() instead)
		a=gettick();
		do{
			sprintf(ctf_pname1,"CTF_%d",a);
			a++;
		}while(party_searchname(ctf_pname1));
		do{
			sprintf(ctf_pname2,"CTF_%d",a);
			a++;
		}while(party_searchname(ctf_pname2));

		//process players
		j=0;
		for(i=0 ; i<maxp ; i++){
			if(are_queue_first[2]==NULL){ break; }
			if(are_queue_first[2]->sd->pd!=NULL) pet_return_egg(are_queue_first[2]->sd, are_queue_first[2]->sd->pd);
			//auto party-management
			if(are_queue_first[2]->sd->status.party_id){ party_leave(are_queue_first[2]->sd); are_queue_first[2]->sd->status.party_id=0; }
			if(j==0){
				//leader
				party_create(are_queue_first[2]->sd,ctf_pname1,0,0);
				//ctf_pid1=are_queue_first[2]->sd->status.party_id;
				//mapreg_setreg(add_str("$CTF_teamA_id"), ctf_pid1);
				sprintf(out,"$CTF%d_teamA_leader",arena+1);
				mapreg_setreg(add_str(out), are_queue_first[2]->sd->status.account_id);
				pc_setreg(are_queue_first[2]->sd, add_str("@CTF_team"), 1);
			}else if(j<maxp/2){
				//join
				//party_fill_member(&member, are_queue_first[2]->sd);
				//intif_party_addmember(ctf_pid1, &member);
				pc_setreg(are_queue_first[2]->sd, add_str("@CTF_team"), 1);
			}else if(j==maxp/2){
				//leader
				party_create(are_queue_first[2]->sd,ctf_pname2,0,0);
				//ctf_pid2=are_queue_first[2]->sd->status.party_id;
				//mapreg_setreg(add_str("$CTF_teamB_id"), ctf_pid2);
				sprintf(out,"$CTF%d_teamB_leader",arena+1);
				mapreg_setreg(add_str(out), are_queue_first[2]->sd->status.account_id);
				pc_setreg(are_queue_first[2]->sd, add_str("@CTF_team"), 2);
			}else if(j<maxp){
				//join
				//party_fill_member(&member, are_queue_first[2]->sd);
				//intif_party_addmember(ctf_pid2, &member);
				pc_setreg(are_queue_first[2]->sd, add_str("@CTF_team"), 2);
			}
			//arena var
			pc_setreg(are_queue_first[2]->sd, add_str("@CTF_arena"), arena);
			...
			if(j<maxp/2){
				...
				x=88+3*(j-(maxp/2)); y=22;
			}
			ret = pc_setpos(are_queue_first[2]->sd,mapindex_name2id(str),x,y,0);
			//if(ret) i--;
			are_queue_first[2]->sd->my_are_fifo3=NULL;
			fifo_del(are_queue_first[2],2);
			j++;
		...
		j=0;
		fifo=are_queue_first[2];
		len=strlen("You have 10000 people ahead of you to enter Capture the flag.");
		while(fifo!=NULL){
			if(fifo->sd==NULL){ fifo=fifo->next; continue; }
			sprintf(out, "You have %d people ahead of you to enter Capture the flag.", j);
			clif_disp_onlyself(fifo->sd,out,len);
			fifo=fifo->next; j++;
		}
		script_pushint(st, i);
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(petclone,""),//nuhuh
	BUILDIN_DEF(petclonetype,"i"),//nuhuh
...
// <--- [zBuffer] List of mob control commands
	...
	// KNX (og2)
	BUILDIN_DEF(getpartymembercount,"i"),//knx
	BUILDIN_DEF(getpartycount,"i"),//knx
	BUILDIN_DEF(countpartyin,"si"),//knx
	BUILDIN_DEF(party_leave,"i"),//knx
	BUILDIN_DEF(party_join,"si"),//knx
	BUILDIN_DEF(party_break,"i"),//knx
	BUILDIN_DEF(ctf_init,"i"),//knx
	BUILDIN_DEF(ctf_lockflag,"iii"),//knx
	BUILDIN_DEF(ctf_unlockflag,"iii"),//knx
	BUILDIN_DEF(sc_get,"i"),//knx
	BUILDIN_DEF(alive,""),//knx

	BUILDIN_DEF(arena_queue_add,"i"),//knx
	BUILDIN_DEF(arena_queue_del,"i"),//knx
	BUILDIN_DEF(warp_arena_queue,"iisiii"),//knx
